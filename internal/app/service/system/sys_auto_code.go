...
import (
	...
	"go-admin/internal/app/resource/autocode_template/subcontract"
...
func Init(Package string) {
	injectionPaths = []injectionMeta{
		{
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, global.SYS_CONFIG.AutoCode.SInitialize, "gorm.go"),
			funcName:    "MysqlTables",
			structNameF: Package + ".%s{},",
		},
		{
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, global.SYS_CONFIG.AutoCode.SInitialize, "router.go"),
			funcName:    "Routers",
			structNameF: Package + "Router.Init%sRouter(PrivateGroup)",
		},
		{
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SApi, Package), "enter.go"),
			funcName:    "ApiGroup",
			structNameF: "%sApi",
		},
		{
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SRouter, Package), "enter.go"),
			funcName:    "RouterGroup",
			structNameF: "%sRouter",
		},
		{
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SService, Package), "enter.go"),
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, "service", "enter.go"),
			importCodeF:  "go-admin/internal/app/%s/%s",
			packageNameF: "%s",
		...
		packageRouterName: {
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, "router", "enter.go"),
			importCodeF:  "go-admin/internal/app/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, "api/v1", "enter.go"),
			importCodeF:  "go-admin/internal/app/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
		...
		var gormPath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
			global.SYS_CONFIG.AutoCode.Server, global.SYS_CONFIG.AutoCode.SInitialize, "gorm.go")
		var routePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
			global.SYS_CONFIG.AutoCode.Server, global.SYS_CONFIG.AutoCode.SInitialize, "router.go")
		var imporStr = fmt.Sprintf("go-admin/internal/app/model/%s", autoCode.Package)
...
func (autoCodeService *AutoCodeService) DropTable(tableName string) error {
	return global.SYS_DB.Exec("DROP TABLE " + tableName).Error
}
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		if strings.Contains(fileSlice[n-2], "router") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root, global.SYS_CONFIG.AutoCode.Server,
				fmt.Sprintf(global.SYS_CONFIG.AutoCode.SRouter, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "api") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SApi, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "service") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SService, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "model") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SModel, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "request") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SRequest, data.autoPackage), base)
		}
	} else if strings.Contains(fileSlice[1], "web") {
		if strings.Contains(fileSlice[n-1], "js") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Web, global.SYS_CONFIG.AutoCode.WApi, base)
		} else if strings.Contains(fileSlice[n-2], "form") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Web, global.SYS_CONFIG.AutoCode.WForm, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
		} else if strings.Contains(fileSlice[n-2], "table") {
			data.autoMoveFilePath = filepath.Join(global.SYS_CONFIG.AutoCode.Root,
				global.SYS_CONFIG.AutoCode.Web, global.SYS_CONFIG.AutoCode.WTable, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), base)
		}
	}
}
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for i, s := range pendingTemp {
		pendingTemp[i].path = filepath.Join(global.SYS_CONFIG.AutoCode.Root, global.SYS_CONFIG.AutoCode.Server, filepath.Clean(fmt.Sprintf(s.path, packageName)))
	}
	// 选择模板
...
func (autoCodeService *AutoCodeService) CreatePlug(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[2], "tpl") < 0 {
			dirPath := filepath.Join(global.SYS_CONFIG.AutoCode.Root, global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SPlug, plug.Snake+"/"+pathArr[2]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.SYS_CONFIG.AutoCode.Root, global.SYS_CONFIG.AutoCode.Server, fmt.Sprintf(global.SYS_CONFIG.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugPath):len(tpl)-4]))
