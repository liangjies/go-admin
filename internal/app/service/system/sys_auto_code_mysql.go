...
func (s *autoCodeMysql) GetDB() (data []response.Db, err error) {
	...
	err = global.SYS_DB.Raw(sql).Scan(&entities).Error
	return entities, err
}
...
func (s *autoCodeMysql) GetTables(dbName string) (data []response.Table, err error) {
	...
	err = global.SYS_DB.Raw(sql, dbName).Scan(&entities).Error
	return entities, err
}
...
func (s *autoCodeMysql) GetColumn(tableName string, dbName string) (data []response.Column, err error) {
	...
	WHERE table_name = ?
	  AND table_schema = ?
	`
	err = global.SYS_DB.Raw(sql, tableName, dbName).Scan(&entities).Error
	return entities, err
}
