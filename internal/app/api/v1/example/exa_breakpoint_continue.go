...
import (
	...
	exampleRes "go-admin/internal/app/model/example/response"
...
func (u *FileUploadAndDownloadApi) BreakpointContinue(c *gin.Context) {
	...
	if err != nil {
		global.SYS_LOG.Error("接收文件失败!", zap.Error(err))
		response.FailWithMessage("接收文件失败", c)
		return
	}
	f, err := FileHeader.Open()
	if err != nil {
		global.SYS_LOG.Error("文件读取失败!", zap.Error(err))
...
func (u *FileUploadAndDownloadApi) FindFile(c *gin.Context) {
	...
	err, file := fileUploadAndDownloadService.FindOrCreateFile(fileMd5, fileName, chunkTotal)
	if err != nil {
		global.SYS_LOG.Error("查找失败!", zap.Error(err))
...
func (b *FileUploadAndDownloadApi) BreakpointContinueFinish(c *gin.Context) {
	...
	if err != nil {
		global.SYS_LOG.Error("文件创建失败!", zap.Error(err))
