...
func (e *ExcelApi) ExportExcel(c *gin.Context) {
	...
	filePath := global.SYS_CONFIG.Excel.Dir + excelInfo.FileName
	err := excelService.ParseInfoList2Excel(excelInfo.InfoList, filePath)
	if err != nil {
		global.SYS_LOG.Error("转换Excel失败!", zap.Error(err))
...
func (e *ExcelApi) ImportExcel(c *gin.Context) {
	...
	_ = c.SaveUploadedFile(header, global.SYS_CONFIG.Excel.Dir+"ExcelImport.xlsx")
...
func (e *ExcelApi) LoadExcel(c *gin.Context) {
	...
	if err != nil {
		global.SYS_LOG.Error("加载数据失败!", zap.Error(err))
...
func (e *ExcelApi) DownloadTemplate(c *gin.Context) {
	...
	filePath := global.SYS_CONFIG.Excel.Dir + fileName

	fi, err := os.Stat(filePath)
	if err != nil {
		global.SYS_LOG.Error("文件不存在!", zap.Error(err))
		response.FailWithMessage("文件不存在", c)
		return
	}
	if fi.IsDir() {
		global.SYS_LOG.Error("不支持下载文件夹!", zap.Error(err))
