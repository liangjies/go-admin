...
func (autoApi *AutoCodeApi) PreviewTemp(c *gin.Context) {
	...
	if err != nil {
		global.SYS_LOG.Error("预览失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) CreateTemp(c *gin.Context) {
	...
	if a.AutoCreateApiToSql {
		if ids, err := autoCodeService.AutoCreateApi(&a); err != nil {
			global.SYS_LOG.Error("自动化创建失败!请自行清空垃圾数据!", zap.Error(err))
...
func (autoApi *AutoCodeApi) GetTables(c *gin.Context) {
	dbName := c.DefaultQuery("dbName", global.SYS_CONFIG.Mysql.Dbname)
	tables, err := autoCodeService.Database().GetTables(dbName)
	if err != nil {
		global.SYS_LOG.Error("查询table失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) CreatePackage(c *gin.Context) {
	...
	if err != nil {
		global.SYS_LOG.Error("创建成功!", zap.Error(err))
